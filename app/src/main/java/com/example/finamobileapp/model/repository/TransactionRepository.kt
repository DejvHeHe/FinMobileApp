...
class TransactionRepository(private val transactionDao: TransactionDao) {
            ...
            if (transaction.accountType == TransactionAccountType.REGULAR) {
                transactionDao.addTransaction(
                    transaction.copy(
                        ...
                        accountType = TransactionAccountType.SAVINGS,
                ...
                transactionDao.addTransaction(
                    transaction.copy(
                        type = TransactionType.INCOME,
                        accountType = TransactionAccountType.REGULAR
                    )
                )
                transactionDao.addTransaction(transaction)
            }
    ...
    suspend fun addRecurringTransactions(transaction: Transaction, endDate: LocalDate) {
        ...
        val monthsToCreate =
            ChronoUnit.MONTHS.between(startDate.withDayOfMonth(1), endDate.withDayOfMonth(1))
